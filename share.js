  ...
  radarMap.on('load', () => {
    ...
    items.forEach((item, index) => {
      ...
      if (item.latitude && item.longitude) {
        ...
        // Create compact popup HTML
        let popupHTML = `
          <div style="text-align: center; padding: 4px; max-width: 180px;">
            <div style="background: rgba(66, 167, 70, 0.1); border: 1px solid rgba(66, 167, 70, 0.3); color: #42A746; padding: 2px 6px; border-radius: 8px; font-size: 11px; font-weight: 600; display: inline-block; margin-bottom: 6px;">
              ${index + 1}
            </div>
            <h3 style="margin: 4px 0; font-size: 14px; font-weight: 600; color: #000000; line-height: 1.3;">${venue}</h3>
        `;

        if (imageUrl) {
          popupHTML += `<img src="${imageUrl}" style="width: 100%; max-width: 180px; height: auto; border-radius: 6px; margin: 6px 0; display: block;" />`;
        }

        if (author) {
          popupHTML += `<div style="font-size: 11px; color: #666; margin: 2px 0;">${author}</div>`;
        }

        if (eventDate) {
          popupHTML += `<div style="font-size: 11px; color: #666; margin: 2px 0;">${eventDate}</div>`;
        }

        if (item.url) {
          popupHTML += `<a href="${item.url}" target="_blank" style="display: inline-block; margin-top: 6px; padding: 6px 12px; background: #42A746; color: white; text-decoration: none; border-radius: 6px; font-size: 11px; font-weight: 600;">View Post</a>`;
        }
